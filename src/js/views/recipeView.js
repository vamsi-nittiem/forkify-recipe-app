...
class RecipeView extends View {
    ...
    addUpdateServingsHandler(servingsHandler){
      this._parentElem.addEventListener('click', (event) => {
        const btnEl = event.target.closest('.btn--increase-servings');
        if(!btnEl) return;
        if(+btnEl.dataset.updatevalue > 0)
          servingsHandler(+btnEl.dataset.updatevalue);
      })
    }

    addBookmarkClickHandler(handler){
      this._parentElem.addEventListener('click', (event) => {
        const btnEl = event.target.closest('.btn--bookmark');
        if(!btnEl) return;
        handler();
      })
    }
        ...
        <div class="recipe__details">
          ...
          <div class="recipe__info">
            ...
            <div class="recipe__info-buttons">
              <button class="btn--tiny btn--increase-servings" data-updatevalue=${this._data.servings - 1}>
                <svg>
